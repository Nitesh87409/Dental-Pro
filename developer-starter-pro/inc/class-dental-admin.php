...
class Developer_Starter_Pro_Admin {
	...
	public function sanitize_options( $input ) {
		...
		if ( empty( $active_tab ) || 'chatbot' === $active_tab ) {
			// Chatbot Settings.
			$sanitized['chatbot_enable']        = isset( $input['chatbot_enable'] ) ? '1' : '0';
			$sanitized['chatbot_api_url']       = isset( $input['chatbot_api_url'] ) ? esc_url_raw( $input['chatbot_api_url'] ) : '';
				...
				<form method="post" action="options.php" id="developer-starter-pro-settings-form">
					...
					switch ( $active_tab ) {
						...
						case 'appointments':
							...
							break;
						case 'chatbot':
							$this->render_tab_chatbot( $options );
							break;
					}
					?>
	...
	/**
	 * Render Chatbot Settings Tab.
	 *
	 * @param array $options Theme options.
	 */
	private function render_tab_chatbot( $options ) {
